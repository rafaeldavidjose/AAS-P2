...
# Agent factory
def make_agent():
    return HeuristicAgent(debug=False)
...
def main():
    ...
    # Save metrics to CSV
    csv_path = os.path.join(OUT_DIR, "heuristic_metrics.csv")
    ...
    np.save(os.path.join(OUT_DIR, "heuristic_heatmap.npy"), heatmap_array)
    ...
    plt.title("Heuristic Agent - Movement Heatmap")

    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "heuristic_heatmap.png"), dpi=200)
    plt.close()
    ...
    print("=" * 70)
    print("Benchmark Summary - Heuristic Agent")
